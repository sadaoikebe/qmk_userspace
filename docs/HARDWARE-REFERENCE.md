...
| TC69 | ⚪ 資料化（アルゴリズムのスタブ付き） | 静電容量。XIAO 子基板。**⚠️ ファームウェアのバックアップが無い（§8）** |
...
> ## ⚠️ TC69 のファームウェアはバックアップが存在しない
>
> 実機は手元にあるが、**そこに載っている flash が、すぐ使える唯一の TC69 ファームウェア**
> である可能性が高い。理由:
>
> * **ビルド済みバイナリがどこにも無い。** `qmk-archive/known-good/` にあるのは TS52K の 3 本だけで、
>   旧リポジトリにコミットされていた `.hex` も `ts52k_nicola4r` と `xd60` 系だけだった
> * ソースのある `capacitive` / `tc69` ブランチは、**どちらもアンカータグ
>   `archive/oldfork-final` の祖先ではない**。タグからは辿れず、**bundle の中にしか無い**
> * `C:\Users\marur\qmk_firmware_oldfork` は削除済みなので作業ツリーも残っていない
> * 再ビルドは「bundle から clone → `capacitive` を checkout → **2023-04 世代の QMK** を
>   現在のツールチェーンでビルド」になる。**通る保証は無い**
>
> **やるべきこと: 先に吸い出す。** RP2040 は読み出し保護がかかっていないので flash をそのまま抜ける。
>
> ```
> BOOTSEL を押しながら USB に挿す → picotool save --all tc69_backup.uf2
> ```
>
> **この XIAO を他の用途に流用しないこと。** RP2040 は BOOTSEL がマスク ROM にあるので
> 基板が文鎮化することはないが、**上書きした時点でファームウェアは消える。**
> XIAO RP2040 は ¥1,000 程度なので、実験には別途買う。

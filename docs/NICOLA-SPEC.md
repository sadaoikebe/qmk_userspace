...
> ⚠️ **読むときの注意（2026-08-12 追記）。** ここでいう `users/nicola/` は
> **旧フォーク（`archive/oldfork-final`）にあった TS69 世代の実装**を指す。
> 書き直し後は**同じ `users/nicola/` というパスに新しい実装が入っている**ので混同しないこと。
> 現在の `users/nicola/` は `nicola4r` を書き直したもので、廃止対象ではない。

| 対象 | 理由 |
|---|---|
| **旧フォークの** `users/nicola/` 一式 | `nicola4r` が上位互換。TS69 は keymap 側ガードで同じ挙動になる |

...
> ⚠️ **この既知の動作品セットは TS52K だけをカバーしている。**
> 他の機種にはビルド済みバイナリが無い。特に **TC69 はファームウェアのバックアップが存在せず、
> ソースも bundle 単独依存**なので、実機の flash を上書きすると失われる。
> 詳細と吸い出し手順は [`HARDWARE-REFERENCE.md`](HARDWARE-REFERENCE.md) §8。
